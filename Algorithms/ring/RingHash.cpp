...
#include <cstdio>
#include "../../../../../../Downloads/cpp-consistent-hashing-algorithms-ring/Algorithms/misc/HashFunctions.h"
#include "RingHash.h"

void RingHash::insert(uint32_t nodeID) {
    //for (uint32_t i = 0; i < V; i++) {
        uint32_t key2 = generate32RandomNumber(0);
        uint32_t hashValue = crc32(nodeID, key2);
...
uint32_t RingHash::getNodeID(uint32_t key, uint32_t* numHash) {
    ...
    uint32_t hashValue = crc32(key, key2);
